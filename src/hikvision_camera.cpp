...
HikvisionCamera::HikvisionCamera()
{
    ...
    strncpy(_struLoginInfo.sDeviceAddress, "192.168.80.171", NET_DVR_DEV_ADDRESS_MAX_LEN - 1);
    _struLoginInfo.sDeviceAddress[NET_DVR_DEV_ADDRESS_MAX_LEN - 1] = '\0'; // Ensure null-termination
    strncpy(_struLoginInfo.sUserName, "admin", NAME_LEN - 1);
    _struLoginInfo.sUserName[NAME_LEN - 1] = '\0'; // Ensure null-termination
    strncpy(_struLoginInfo.sPassword, "Pass@word1", NAME_LEN - 1);
    _struLoginInfo.sPassword[NAME_LEN - 1] = '\0'; // Ensure null-termination

    printf("Start to login Remote Camera\n");
...
int HikvisionCamera::Demo_Capture()
{
    ...
    strncpy(struLoginInfo.sDeviceAddress, "192.168.72.145", NET_DVR_DEV_ADDRESS_MAX_LEN - 1);
    struLoginInfo.sDeviceAddress[NET_DVR_DEV_ADDRESS_MAX_LEN - 1] = '\0'; // Ensure null-termination
    strncpy(struLoginInfo.sUserName, "admin", NAME_LEN);
    strncpy(struLoginInfo.sPassword, "Pass@word1", NAME_LEN);
    ...
    int iRet;
    char jpegFilePath[] = "./ssss.jpeg";
    iRet = NET_DVR_CaptureJPEGPicture(lUserID, struDeviceInfoV40.struDeviceV30.byStartChan, &strPicPara, jpegFilePath);
    if (!iRet)
    {
...
{
    return PtzControlAndStop(ptzCommand, 200);
}
...
{
    return PtzControl(ptzCommand, 0);
}

int HikvisionCamera::PtzControlStop(DWORD ptzCommand)
{
    return PtzControl(ptzCommand, 1);
}
...
{
    return InvokeISAPI(method, apiPath, "");
}
...
int HikvisionCamera::GetStream(REALDATACALLBACK fRealDataCallBack_V30)
{
    ...
    int iRealPlayHandle = NET_DVR_RealPlay_V40(_lUserID, &struPreviewInfo, fRealDataCallBack_V30, NULL);
    if (iRealPlayHandle >= 0)
    {
        printf("[GetStream]---RealPlay success, \n");
    }
    else
    {
        printf("[GetStream]---RealPlay failed, error = %d\n", NET_DVR_GetLastError());
    }
